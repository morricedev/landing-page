import { mapImageGrid, mapSectionContent, mapSections, mapSectionTwoColumns, mapTextGrid } from './mapSections';
...
describe('mapSections', () => {
  ...
  it('should test section with invalid data', () => {
    ...
    expect(withNoTextOrImageGrid).toEqual(withNoTextOrImageGrid);
    expect(withNoComponent).toEqual(withNoComponent);
  });

  it('should test section.section-grid with no text or no images', () => {
    const noTextGrid = mapSections([{ __component: 'section.section-grid', image_grid: ['a.svg'] }]);
    const noImageGrid = mapSections([{ __component: 'section.section-grid', text_grid: ['teste'] }]);

    expect(noTextGrid[0].component).toBe('section.section-grid-image');
    expect(noImageGrid[0].component).toBe('section.section-grid-text');
  });
